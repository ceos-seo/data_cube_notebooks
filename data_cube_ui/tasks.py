...
def generate_chunk(time_num,
    ...
    while time_index < len(acquisition_list):
        ...
        if processing_options['time_slices_per_iteration'] is not None and (
                time_index + processing_options['time_slices_per_iteration'] - 1) < len(acquisition_list):
            end = acquisition_list[time_index + processing_options['time_slices_per_iteration'] - 1]
        else:
            end = acquisition_list[-1] if processing_options['reverse_time'] else acquisition_list[
        ...
        if raw_data is None or "cf_mask" not in raw_data:
            time_index = time_index + (processing_options['time_slices_per_iteration']
                                       if processing_options['time_slices_per_iteration'] else len(acquisition_list))
            continue

        animation_data = {
            'type': query.animated_product,
            'path': processing_options['base_path'] + query.query_id + '/' + str(time_num) + '/',
            'chunk_num': chunk_num,
            'time_offset': time_index
        } if hasattr(query, 'animated_product') and query.animated_product != "None" else None
    ...
    if chunk_data is None or "cf_mask" not in chunk_data or not os.path.exists(processing_options['base_path'] +
                                                                               query.query_id):
        return None
