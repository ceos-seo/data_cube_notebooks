...
from .models import PixelPlottingTask
...
class BaseTask(DCAlgorithmBase):
    # TODO: replace the __app__name var with pixel_plotting - avoids the auto rename.
    __app__name = 'pixel_plotting'


@task(name="pixel_plotting.run", base=BaseTask)
...
@task(name="pixel_plotting.parse_parameters_from_task", base=BaseTask)
def parse_parameters_from_task(task_id=None):
    ...
    """
    task = PixelPlottingTask.objects.get(pk=task_id)
...
@task(name="pixel_plotting.validate_parameters", base=BaseTask)
...
@task(name="pixel_plotting.perform_task_chunking", base=BaseTask)
...
@task(name="pixel_plotting.start_chunk_processing", base=BaseTask)
...
@task(name="pixel_plotting.processing_task", acks_late=True, base=BaseTask)
...
@task(name="pixel_plotting.recombine_geographic_chunks", base=BaseTask)
...
@task(name="pixel_plotting.recombine_time_chunks", base=BaseTask)
...
@task(name="pixel_plotting.create_output_products", base=BaseTask)
