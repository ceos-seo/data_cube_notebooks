...
def create_query_from_post(user_id, post):
    ...
    query = Query(query_start=datetime.now(), query_end=datetime.now(), user_id=user_id,
                  ...
                  platform=post['platform'], baseline=post['baseline_selection'],
                  baseline_length=post['baseline_length'], area_id=post['area_id'])

    query.title = query.get_baseline_name() + " Baseline SLIP" if 'title' not in post or post['title'] == '' else post['title']
