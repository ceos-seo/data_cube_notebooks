...
def create_query_from_post(user_id, post):
    ...
    query = Query(
        ...
        baseline=post['baseline_selection'],
        baseline_length=post['baseline_length'],
        area_id=post['area_id'])

    query.title = query.get_baseline_name() + " Baseline SLIP" if 'title' not in post or post['title'] == '' else post[
        'title']
