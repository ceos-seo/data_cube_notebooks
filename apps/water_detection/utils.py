...
def create_query_from_post(user_id, post):
    ...
    query = Query(query_start=datetime.now(), query_end=datetime.now(), user_id=user_id,
                  ...
                  time_start=start, time_end=end, platform=post['platform'],
                  animated_product=post['animated_product'], area_id=post['area_id'])

    query.title = query.get_type_name() + " Background WOfS" if 'title' not in post or post['title'] == '' else post['title']
