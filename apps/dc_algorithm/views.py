...
class GetTaskResult(View, ToolClass):
    ...
    def get(self, request):
        """Get a JsonResponse containing a status msg and Task model if complete

        Check on the execution status of a running query by an id provided in the GET parameters
        Default msg should be wait - if there is an error in getting the task, or an error in the task
...
class SubmitNewSubsetRequest(View, ToolClass):
    ...
    def post(self, request):
        ...
        task_model = self._get_tool_model(self._get_task_model_name())
        response = {'msg': "OK"}
        requested_task = tool_model.get(pk=request.POST['id'])
...
class CancelRequest(View, ToolClass):
    ...
    def get(self, request):
        """Get a JsonResponse containing a status msg signifying task removal
